...
// 'Process1_Sym', 8x8px
const unsigned char Process1_Sym [] = {
...
// 'Process2_Sym', 8x8px
const unsigned char Process2_Sym []  = {
...
const unsigned char Clear_Daylight_Sym []  = {
...
const unsigned char Clear_Night_Sym []  = {
...
const unsigned char Thunderstorm_Sym []  = {
...
// 'Drizzle', 24x24px
const unsigned char Drizzle_Sym []  = {
...
const unsigned char Clouds_Daylight_Sym []  = {
  0x00, 0x18, 0x00, 0x00, 0x18, 0x00, 0x10, 0x18, 0x08, 0x38, 0x00, 0x1c, 0x1c, 0x3c, 0x38, 0x08, 
...
const unsigned char Clouds_Night_Sym []  = {
...
// 'Cloudy_Sym', 24x24px
const unsigned char Cloudy_Sym []  = {
...
static void task_write_data(void *data)
{
    ...
    for (;;)
    {
	    ...
	    memset(s_buffer, 0, max_buf_size*sizeof(s_buffer[0]));
	    sh1106_display_clear(NULL);
            snprintf(s_buffer, 100, "      %c%c      %c\nTemp:    %+3.1fC\n \nHumidity:  %3d%%\n \nPressure:%4dhP", ANTENNA_SYMBOL, sig, (mqtt.connected ? MQTT_SYMBOL : ' '), weather->temperature, (int)weather->humidity, (int)reference_pressure);
            //snprintf(s_buffer, 100, "      %c%c      %c\n \nPressure:%lu hP", ANTENNA_SYMBOL, sig, (mqtt.connected ? MQTT_SYMBOL : ' '), reference_pressure);
            //snprintf(s_buffer, 100, "      %c%c      %c\n \nTemp:    %+3.1fK\n \nHumidity:  %3d\n \nPressure:%lu hP", ANTENNA_SYMBOL, sig, (mqtt.connected ? MQTT_SYMBOL : ' '), weather->temperature, weather->humidity, reference_pressure);
	    #endif
